...
func TestUserDomainInService_LoginUserServices(t *testing.T) {
	...
	t.Run("when_sending_a_valid_user_and_userId_returns_success", func(t *testing.T) {
		...
		userDomain := model.NewUserDomain("test@test.com", "test", "test", 44)
		userDomain.SetID(id)
		...
		).Return(nil, rest_error.NewInternalServerError("error trying to find user by email and password"))
	...
	t.Run("when_calling_create_token_returns_error", func(t *testing.T) {
		...
		userDomainMock.EXPECT().GenerateToken().Return("", rest_error.NewInternalServerError("error trying to create token"))
